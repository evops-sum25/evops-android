...
class SuggestTagsByDescriptionUseCase
@Inject
constructor(private val createEventNetworkRepository: CreateEventNetworkRepository) {
    operator fun invoke(description: String): Flow<Resource<List<CreateEventTag>>> = flow {
        try {
            emit(Resource.Loading())
            val tags = createEventNetworkRepository.suggestTagsByDescription(description)

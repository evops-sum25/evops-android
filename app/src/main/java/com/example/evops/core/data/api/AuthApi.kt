...
interface AuthApi {
    @POST("/v1/auth/login")
    suspend fun login(@Body credentials: CredentialsWrapperDto): TokensWrapperDto

    @POST("/v1/auth/refresh")
    suspend fun refresh(@Body refreshToken: RefreshTokenWrapperDto): TokensWrapperDto

    @POST("/v1/auth/signup") suspend fun signup(@Body form: SignupFormWrapperDto): TokensWrapperDto
}
